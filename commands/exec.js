...
if(!kode) return msg.channel.send('No parameters to execute. 🤦‍♂️');
exec(kode, async (err, sonuç, hata) => {
    
if(sonuç){
if(sonuç.length > 2047) {
hastebin(sonuç, { extension: "js" }).then(cıkra => { msg.channel.send('An error has occurred.'+cıkra+''); })
return
}
...
if(hata.length > 2047) {
 hastebin(hata, { extension: "js" }).then(cıkra => { msg.channel.send('An error has occurred. '+cıkra+''); })
return
 }
 Gönder(hata, msg.channel.id)

}else{
 msg.channel.send('The command executed successfully but returned no output.')
}
